...
.PHONY: check
check: node_modules
	@${BIN}/eslint src test/tests --ext .ts --max-warnings 0 --format unix && echo "Ok"

.PHONY: format
format: node_modules
	@${BIN}/eslint src test/tests --ext .ts --fix

.PHONY: publish
publish: | distclean node_modules lib
	@git diff-index --quiet HEAD || (echo "Uncommitted changes, please commit first" && exit 1)
	@git fetch origin && git diff origin/main --quiet || (echo "Changes not pushed to origin, please push first" && exit 1)
	@pnpm publish --access public && git push && git push --tags
